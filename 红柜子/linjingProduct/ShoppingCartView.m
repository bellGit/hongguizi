...
-(void)layoutUI
{
    ...
    //购物金额提示框
    _money = [[UILabel alloc] initWithFrame:CGRectMake(77, 8, self.bounds.size.width, 30)];
    [_money setTextColor:[UIColor whiteColor]];
    [_money setText:@"¥ 0"];
    [_money setFont:[UIFont systemFontOfSize:21.0]];
    [self addSubview:_money];
    
    //结账按钮
    _accountBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    _accountBtn.layer.cornerRadius = 0;
    _accountBtn.titleLabel.font = [UIFont boldSystemFontOfSize:15];
    _accountBtn.frame = CGRectMake(self.bounds.size.width - 106, 0, 106,50);
    ...
    [_shoppingCartBtn setBackgroundImage:[UIImage imageNamed:@"carwhite@2x.png"] forState:UIControlStateNormal];
...
-(void)setTotalMoney:(NSInteger)nTotal
{
    ...
    else
    {
        ...
        _accountBtn.enabled = NO;
        [_accountBtn setTitle:[NSString stringWithFormat:@"还差￥%ld",(unsigned long)_minFreeMoney] forState:UIControlStateNormal];
        [_accountBtn setBackgroundColor:[UIColor grayColor]];
